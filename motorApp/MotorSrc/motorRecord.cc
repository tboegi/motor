...
/* SPAM bits:
    ...
    2 MIP, DMOV or MSTA changes,  may be retry, doRetryOrDone, delayReq/Ack
...
static long init_re_init(motorRecord *pmr, bool initcall)
{
    ...
    if (pmr->priv->motorSoftLimits.motorROdialLimitsValid ||
        pmr->priv->motorSoftLimits.motorRWdialLimitsValid)
    {
        pmr->dhlm = pmr->priv->motorSoftLimits.motorDialHighLimit;
        pmr->dllm = pmr->priv->motorSoftLimits.motorDialLowLimit;
    }
...
static void maybeRetry(motorRecord * pmr)
{
    ...
    msta_field msta;
    msta.All = pmr->msta;
    int has_problem = msta.Bits.RA_PROBLEM;
    int has_slip_stall = msta.Bits.EA_SLIP_STALL;
...
#ifdef DEBUG
    {
      ...
      dbgMipToString(pmr->mip, dbuf, sizeof(dbuf));
      Debug(pmr,2,
            "maybeRetry: %s%s%s%scommandedDval=%f last.dval=%f dval=%f drbv=%f rdbd=%f diff=%f rcnt=%d pmr->rtry=%d mip=0x%0x(%s)\n",
            (ls_active || has_problem || has_slip_stall) ? "" : close_enough ? "close enough " : "not close enough ",
            ls_active ? "ls_active " :"",
            has_problem ? "has_problem " : "",
            has_slip_stall ? "slip_stall " : "",
    ...
    if (!close_enough)
    {
        if (ls_active || has_problem || has_slip_stall)
        {
...
static long process(dbCommon *arg)
{
    ...
    if (process_reason == CALLBACK_DATA_SOFT_LIMITS)
    {
        if (pmr->priv->motorSoftLimits.motorROdialLimitsValid ||
            pmr->priv->motorSoftLimits.motorRWdialLimitsValid)
        {
            double maxValue = pmr->priv->motorSoftLimits.motorDialHighLimit;
            double minValue = pmr->priv->motorSoftLimits.motorDialLowLimit;
            Debug(pmr,3,
...
static long special(DBADDR *paddr, int after)
{
    ...
    case motorRecordACCL:
        /* ESS special: When a user writes to ACCL, it shoud be used */
        if (pmr->accu != motorACCU_Accl)
        {
            pmr->accu = motorACCU_Accl;
    ...
    case motorRecordACCS:
        /* ESS special: When a user writes to ACCS, it shoud be used */
        if (pmr->accu != motorACCU_Accs)
        {
            pmr->accu = motorACCU_Accs;
            db_post_events(pmr, &pmr->accu, DBE_VAL_LOG);
        }
        if (pmr->accs <= 0.0)
        {
...
static void check_speed_and_resolution(motorRecord * pmr)
{
    ...
    if (pmr->mres > 0)
    {
        ...
        if (pmr->rllm != 0.0)
        {
            pmr->dllm = devSupRawToDial(pmr, pmr->rllm);
            MARK(M_DLLM);
        }
        if (pmr->rllm != devSupDialToRaw(pmr, pmr->dllm))
        {
            pmr->rllm = devSupDialToRaw(pmr, pmr->dllm);
            MARK_AUX(M_RLLM);
        }
        /* RHLM <--> DHLM */
        if (pmr->rhlm != 0.0)
        {
            pmr->dhlm = devSupRawToDial(pmr, pmr->rhlm);
            MARK(M_DHLM);
        }
        if (pmr->rhlm != devSupDialToRaw(pmr, pmr->dhlm))
        {
            pmr->rhlm = devSupDialToRaw(pmr, pmr->dhlm);
            MARK_AUX(M_RHLM);
        }
    }
    else
...
static void set_dial_highlimit(motorRecord *pmr)
{
    ...
    if (pmr->priv->motorSoftLimits.motorROdialLimitsValid)
    {
        double motorSoftLimits = pmr->priv->motorSoftLimits.motorDialHighLimit;
        Debug(pmr,3, "pmr->dhlm=%g motorSoftLimits=%g\n", pmr->dhlm, motorSoftLimits);
        if ((pmr->dhlm > motorSoftLimits) || !softLimitsDefined(pmr))
        {
            pmr->dhlm = motorSoftLimits;
        }
    }
    tmp_raw = devSupDialToRaw(pmr, pmr->dhlm);
...
static void set_dial_lowlimit(motorRecord *pmr)
{
    ...
    if (pmr->priv->motorSoftLimits.motorROdialLimitsValid)
    {
        double motorSoftLimits = pmr->priv->motorSoftLimits.motorDialLowLimit;
        Debug(pmr,3, "pmr->dllm=%g motorSoftLimits=%g\n", pmr->dllm, motorSoftLimits);
        if ((pmr->dllm < motorSoftLimits) || !softLimitsDefined(pmr))
        {
            pmr->dllm = motorSoftLimits;
        }
    }
    tmp_raw = devSupDialToRaw(pmr, pmr->dllm);
